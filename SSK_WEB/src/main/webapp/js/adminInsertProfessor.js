document.addEventListener('DOMContentLoaded', () => {
	const form = document.getElementById('adminInsertProfessor');
	...
	form.addEventListener('submit', (event) => {
		...
		let isValid = true;
				...
				if (num === '') {
					...
					numWarn.textContent = '* 교번에 공백은 포함될 수 없습니다.';
					isValid = false;
					if (!focusSet) {
						numCursor.focus();
						focusSet = true;
					}
				} else if (!/^20\d{5}$/.test(num)) {
					numWarn.textContent = '* 교번 양식은 20***** 입니다.';
					isValid = false;
					if (!focusSet) {
						numCursor.focus();
						focusSet = true;
